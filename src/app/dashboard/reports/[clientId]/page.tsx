    ...
    <div className="p-6 max-w-3xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div><span className="text-gray-500">Total Reminders Sent:</span> <span className="font-medium">{logs.filter(l => l.action === "reminder_sent").length}</span></div>
          <div><span className="text-gray-500">Documents Uploaded:</span> <span className="font-medium">{docList.length}</span></div>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {logs.map((log) => (
              <div key={log.id} className="flex gap-3 text-sm">
                <div className="w-40 flex-shrink-0 text-gray-400 text-xs pt-0.5">
                ...
                <div>
                  ...
                  {log.channel && (
                    <p className="text-gray-500 text-xs">via {log.channel}</p>
                  )}
                  {log.message_id && (
                    <p className="text-gray-400 text-xs font-mono">ID: {log.message_id}</p>
                  )}
                </div>
              </div>
            ))}
      ...
      <p className="text-xs text-gray-400">
        This report is generated from an immutable audit log. All timestamps are in IST.
        This document can be presented in client disputes and ICAI disciplinary proceedings.
      </p>
    </div>
  );
}
